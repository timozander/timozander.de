...
export function getPostSlugPath(post: Pick<Post, "id" | "data">): string {
	return slugify(post.data.title || getEntrySlugSegment(post.id));
}
 ...
 * "tokyo-beyond-places"
 */
export function getPostSlug(post: Pick<Post, "id" | "data">): string {
	return getPostSlugPath(post);
}

/**
 * Internal post URL on this site.
 */
export function getPostPermalink(post: Pick<Post, "id" | "data">): string {
	return getAssetPath(`posts/${getPostSlugPath(post)}`);
}
 ...
 * Public post URL.
 *
 * External posts link directly to the original publication.
 */
export function getPostUrl(post: Pick<Post, "id" | "data">): string {
	return post.data.externalUrl ?? getPostPermalink(post);
}
